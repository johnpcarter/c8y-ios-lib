...
public class C8yMockedMyGroups : C8yMyGroups {
    ...
    public func testDevice(_ id: String) -> C8yDevice {
        ...
        return makeDevice(id, name: "Device with id \(id)", model: "c8y_Device", category: .Alarm, status: .AVAILABLE, criticalAlarms: 0, majorAlarms: 0, minorAlarms: 0, warningAlarms: 0)
    }
    ...
    private func setup() {
        ...
        var saintJaques = makeSiteGroup("SJ0001", name: "Ecole Saint Jaques", category: .organisation, organisationType: .School)
        ...
        var device1 = makeDevice("D0001", name: "Device 1 in A", model: "c8y_Device", category: .Alarm, status: .AVAILABLE, criticalAlarms: 0, majorAlarms: 0, minorAlarms: 0, warningAlarms: 0)
        device1.position = C8yManagedObject.Position(lat: 48.850158350000555, lng: 2.345681232918291, alt: 0)
        
        var device2 = makeDevice("D0002", name: "Device 2 in A", model: "c8y_Device", category: .Light, status: .UNAVAILABLE, criticalAlarms: 3, majorAlarms: 2, minorAlarms: 1, warningAlarms: 1)
        ...
        device3.network = C8yAssignedNetwork(false)
        ...
        buildA = buildA.addToGroup(device1)!
        buildA = buildA.addToGroup(device2)!
        buildB = buildB.addToGroup(device3)!
        buildB = buildB.addToGroup(device4)!

        saintJaques = saintJaques.addToGroup(buildA)!
        saintJaques = saintJaques.addToGroup(buildB)!
        ...
        _ = libr.addToGroup(device5)
        _ = libr.addToGroup(device6)
        ...
        shop = shop.addToGroup(device7)!
        
        self.addToMyGroups(saintJaques)
        self.addToMyGroups(libr)
        self.addToMyGroups(shop)
        self.addToMyGroups(car)
    ...
    private func makeSiteGroup(_ c8yId: String, name: String, category: C8yGroupCategory, organisationType: C8yOrganisationCategory) -> C8yGroup {
        ...
        return C8yGroup(c8yId, name: name, category: .organisation, location: nil, notes: "How now brown cow, this is a mocked object")
    }
    ...
    private func makeBuildingGroup(_ c8yId: String, name: String, category: C8yGroupCategory) -> C8yGroup {
       ...
       let newGroup = C8yGroup(c8yId, name: name, category: .building, location: nil, notes: "How now brown cow, this is a mocked object")
    ...
    private func makeDevice(_ c8yId: String?, name: String, model: String, category: C8yDeviceCategory, status: C8yManagedObject.AvailabilityStatus, criticalAlarms: Int, majorAlarms: Int, minorAlarms: Int, warningAlarms: Int) -> C8yDevice {
        ...
        device.wrappedManagedObject.supportedOperations = [C8Y_OPERATION_RESTART, C8Y_OPERATION_COMMAND]
