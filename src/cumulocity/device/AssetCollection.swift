...
public class C8yAssetCollection: ObservableObject {
    ...
    private func _addGroup(_ group: C8yGroup, includeSubGroups: Bool) {
        ...
        self._storeCancellable(GroupLoader(group.wrappedManagedObject, conn: self.connection!, path: nil, includeGroups: includeSubGroups).load()
            ...
            case .finished:
				print("** Group loader for \(group.c8yId) - \(group.name) done")
            }
        }, receiveValue: { (newGroup) in
			print("** Group loader for \(newGroup.c8yId) value received, \(newGroup.children.count)")
                ...
                if (existingObject == nil) {
                    ...
                    for i in self.objects.indices {
                        if (self.objects[i].c8yId == newGroup.c8yId) {
							...
							print("====== Updating group \(object.c8yId), has \(newGroup.children.count)")
    ...
    public class GroupSummary: Identifiable {
        ...
        private var _owner: C8yAssetCollection
        
        public init(_ id: String, name: String, isSelected: Bool, includeSubGroups: Bool, owner: C8yAssetCollection) {
